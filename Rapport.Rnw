...
\appto\frontmatter{\pagestyle{toc}}
...
\usepackage[colorlinks,breaklinks,naturalnames,linkcolor=black,citecolor=black,menucolor=black,pagecolor=black,urlcolor=blue,hypertexnames=false]{hyperref}  % ,backref=page
\usepackage{placeins}
\usepackage{booktabs}
...
% Reset le numéro de section dans chaque partie (à charger après hyperref)
\usepackage{chngcntr}
...
<<setup, include=FALSE, cache=FALSE>>=
	...
	opts_chunk$set(fig.path='figure/', cache.path='cache/Simple-', fig.align='center', fig.width=5,fig.height=5, fig.show='hold', tidy=TRUE, fig.pos='htpb', par=TRUE,comment=NA)
...
@
	...
	% acid kellys peaksea
<<knit-theme, cache=FALSE, echo=FALSE>>=
...
\thispagestyle{empty}
   ...
   \begin{figure}
      ...
      \end{minipage} \hfill
	  \centering\textsc{\Large Université de La Rochelle}\hspace{1cm}
...
\addcontentsline{toc}{part}{Structure d'accueil}
...
\begin{itemize}
	...
	\item Une population en MNO, dans laquelle on retrouve uniquement des individus de ce milieu ci. Aucun individu rattaché à une autre population n'y a été échantillonné.
...
La table \ref{indpop} présente l'assignation des individus à leur population selon l'inférence réalisé à l'aide des logiciels \texttt{Structure}, \citet{pritchard2000}, et \texttt{TESS}, \citet{franccois2010spatially}. Les individus commençant par ttr\_ correspondent à ceux échantillonnés pour cette étude, les autres proviennent de la base de données construite et utilisé par Marie Louis lors de sa thèse \citep{phdLouis2014}.
