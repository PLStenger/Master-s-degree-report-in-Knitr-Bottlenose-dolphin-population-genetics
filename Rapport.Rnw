...
<<moore, echo=FALSE, fig.width=7, fig.height=4, out.width="15cm", out.height="8.6cm", fig.align='center', cache=TRUE, dpi=150, fig.cap="Évolution du coût de séquençage d'un génome humain (données du NHGRI Genome Sequencing Program, www.genome.gov/sequencingcostsdata)", fig.scap="Évolution du coût de séquençage d'un génome humain", fig.lab="moore", fig.pos='htpb', message=FALSE, warning=FALSE>>=
...
gg44 <- ggplot(data = all, aes(x=Year, y=Cost, colour=Source)) + geom_line()
gg44 <- gg44 + scale_y_log10(breaks=10^c(2:10)) 
gg44 <- gg44 + xlab("Années") + ylab("Coût ($)") + theme(legend.title=element_blank(), legend.position='top')
gg44
@
...
<<lositan, echo=FALSE, results='asis', message=FALSE, out.width="14cm", out.height="7cm", fig.width=12, fig.height=6, fig.align='center', cache=TRUE, fig.lab="outlier">>=
library(grid)
library(ggplot2)
j1 <- read.table("batch_1-1.lositan",head=TRUE)
j2 <- read.table("batch_1-2.lositan",head=TRUE)
j3 <- read.table("batch_1-3.lositan",head=TRUE)

j <- rbind(j1,j2,j3)
j$P[j$P==-100] <- 0 
j$Selection <- j$P
j$Selection[j$P>0.99] <- "Sélection positive    "
j$Selection[j$P<0.01] <- "Sélection balancée     "
j$Selection[j$P>=0.01 &j$P<=0.99] <- "Neutre     "
j$Selection <- factor(j$Selection, levels=c("Sélection positive    ","Neutre     ","Sélection balancée     "))

# The palette with grey:
cbPalette <- c("#E69F00","#999999", "#555555" )

g <- ggplot(data=j, aes(x=Het, y=Fst, group=Selection)) + scale_colour_manual(values=cbPalette)
g <- g + geom_point(aes(colour = Selection), alpha = 0.8, size=3)
g <- g + theme(axis.text=element_text(size=18), axis.title=element_text(size=18), legend.key.size=unit(1,"cm"), legend.title=element_blank(), legend.text=element_text(size=18), legend.position="top")
g <- g +  xlab("Hétérozygotie") + ylab(expression(F[ST]))
g
@		
...
\begin {bibunit} [newbst]
\addcontentsline{toc}{part}{Rapport}
...
<<source, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, cache=TRUE>>=
library(ggplot2)
source("LnPD.R")
source("DIC.R")
source("BIC.R")
...
<<lnpd, echo=FALSE,cache=FALSE, warning = FALSE, fig.width = 6, fig.height = 2.2, results='asis', fig.align = 'center', fig.scap="Vraisemblance ($LnPD$) en fonction de $K$.", fig.cap="Vraisemblance ($LnPD$) en fonction de $K$. Les points correspondent aux moyennes de 15 simulations et les barres d'erreurs représentent les écart-types. Le nombre de populations $K$ le plus vraisemblable est celui qui présente une valeur moyenne maximale et un écart-type le plus faible \\citep{pritchard2000}.", fig.pos='htpb', cache=TRUE>>=
gg1
@

<<dic, echo=FALSE,cache=FALSE, warning = FALSE, fig.width = 6, fig.height = 2.2, results='asis', fig.align = 'center', fig.scap="Deviance Information Criterion (DIC) en fonction de $K$.", fig.cap="Deviance Information Criterion (DIC) en fonction de $K$. Les points correspondent aux moyennes de 15 simulations et les barres d'erreurs représentent les écart-types. Le nombre de populations $K$ le plus vraisemblable est celui pour lequel la rupture de pente la plus forte est observée \\citep{durand2009}.", fig.pos='htpb', cache=TRUE>>=
gg2
@


<<bic, echo=FALSE,cache=FALSE, warning = FALSE, fig.width = 6, fig.height = 2.2, results='asis', fig.align = 'center', fig.scap="Bayesian Information Criterion (BIC) en fonction de $K$.", fig.cap="Bayesian Information Criterion (BIC) en fonction de $K$. Le nombre de populations $K$ le plus vraisemblable est celui pour lequel la valeur de BIC est minimale \\citep{jombart2008}.", fig.pos='htpb', cache=TRUE, message=FALSE, warning=FALSE>>=
gg3
@
...
<<barplot, echo=FALSE,cache=FALSE, warning = FALSE, fig.width = 8, fig.height = 2, results='asis', fig.scap="Probabilités d'assignations individuelles à chaque population inférée par TESS", fig.cap="Probabilités d'assignations individuelles à chaque population inférée par TESS. Les individus ont été classés par longitude croissante et les zones géographiques d'origine sont indiquées. Chaque couleur correspond à l'une des 3 populations inférées.", fig.pos='htpb', fig.align = 'center', cache=TRUE, message=FALSE, warning=FALSE>>=

coord <- read.table("microsatcoord_PL + ML (Medit) 2.txt", row.names=1)
coord <- coord[,1:2]
ordered.coord <- coord[order(coord[,2]),]
proba <- read.table("ProbaRun29.txt", head=F)
proba <- proba[,1:3]
ordered.proba <- proba[order(coord[,2]),]
prob <- ordered.proba
rownames(prob) <- rownames(ordered.coord)
site <- rep(c("Galice","Gibraltar","Méditerrannée"), c(33, 39, 49))
prob$site <- site
prob <- prob[,c(4,1:3)]
color <- c("antiquewhite2","cadetblue3","cadetblue4")

par(mar=c(3,5,1,1))
structure.plot(prob, horiz = FALSE, col = color, axis = TRUE, ylab="Probabilité \n d'assignation")
axis(2, las=1)
@
...
<<lnpd2, echo=FALSE,cache=FALSE, warning = FALSE, fig.width = 6, fig.height = 2.2, results='asis', fig.align = 'center', fig.scap="Vraisemblance ($LnPD$) en fonction de $K$.", fig.cap="Vraisemblance ($LnPD$) en fonction de $K$. Les points correspondent aux moyennes de 15 simulations et les barres d'erreurs représentent les écart-types. Le nombre de populations $K$ le plus vraisemblable est celui qui présente une valeur moyenne maximale et un écart-type le plus faible \\citep{pritchard2000}.", fig.pos='htpb', cache=TRUE>>=
gg4
@
...
\rotatebox{90}{
 \begin{tabular}{ p{2cm} p{3cm} p{5cm} p{4cm} p{2cm} p{2cm} p{1.5cm} p{1cm}}
 	\centering
 Marqueurs & \centering	Référence & \centering Primers 5'-3' (R et F) & \centering Motif & \centering Taille des allèles & \centering Concentrations des primers (Tous ou R/F/F* or R/F*) en µM & \centering T° annealing en °C & \centering Longueur d'onde \tabularnewline
 EV37 & \centering \citep{valsecchi1996microsatellite} - \citep{vollmer2012developing} & \centering AGCTTGATTTGGAAGTCATGA GTTTTAGTAGAGCCGTGATAAAGTGC  & \centering (AC)24	& \centering 196-250 & \centering 0.24	& \centering 55	& \centering 800  \tabularnewline
 KMW12a & \centering \citep{hoelzel1998genetic} & \centering CCATACAATCCAGCAGTC CACTGCAGAATGATGACC	& \centering (CA)n & \centering 144-168	& \centering 0.125/0.075/0.05 & \centering 46 & \centering 800  \tabularnewline
 MK5 & \centering \citep{krutzen2001characterization} - \citep{vollmer2012developing}& \centering CTCAGAGGGAAATGAGGCTG GTTTTGTCTAGAGGTCAAAGCCTTCC & \centering (TG)13CT(TG)2CA(TG)2(TA)2(TG)4 & \centering	205-243 & \centering 0.2 & \centering 55 & \centering 700 \tabularnewline
 MK6 & \centering \citep{krutzen2001characterization} - \citep{vollmer2012developing} & \centering GTCCTCTTTCCAGGTGTAGCC GCCCACTAAGTATGTTGCAGC & \centering (GT)17 & \centering 145-191 & \centering 0.2 & \centering	55 & \centering	700  \tabularnewline
 MK8 & \centering \citep{krutzen2001characterization} - \citep{vollmer2012developing} & \centering TCCTGGAGCATCTTATAGTGGC GTTTCTCTTTGACATGCCCTCACC	& \centering (CA)23	& \centering 87-117	& \centering 0.2 & \centering	55	& \centering 800 \tabularnewline
 MK9 & \centering \citep{krutzen2001characterization} -\citep{vollmer2012developing} & \centering CATAACAAAGTGGGATGACTCC GTTTTTATCCTGTTGGCTGCAGTG & \centering (CA)17 & \centering 166-182 & \centering 0.4 & \centering 55 & \centering 800  \tabularnewline
 Tur4-87 & \centering \citep{nater2009new}	& \centering CCCCATATGATGCCTTTGTAAGTCC AATTCCTTGTAACAAACCTCTTTATCT	& \centering (GATA)8 & \centering 182-202 & \centering 0.225/0.225	& \centering 61	& \centering 800  \tabularnewline
 Tur4-98 & \centering \citep{nater2009new}	& \centering GTCCCCAGAACTTAGCACACTGTC CAACTGGGGTCCAAAGAAAGAAG & \centering (GATG)10 & \centering 172-204 & \centering 0.225/0.225 & \centering 63 & \centering 800  \tabularnewline
 Tur4-128 & \centering \citep{nater2009new} & \centering ACGTGCGCATGTCTTTGTCTTAT CTTTGGACGGGGAGTAGAACCTA & \centering	(GATA)11 & \centering 280-304 & \centering 0.225/0.225	& \centering 62	& \centering 800  \tabularnewline
 Tur4-142 & \centering \citep{nater2009new} & \centering GGCCCCCTTTTCCATCCTCA CCAGCCCCCAAAATCACGAGT & \centering (GATA)9 & \centering 320-340 & \centering 0.225/0.225	& \centering 61	& \centering 800  \tabularnewline
 TexVet5 & \centering \citep{rooney1999microsatellite} - \citep{vollmer2012developing} & \centering GTCCTCTTTCCAGGTGTAGCC GCCCACTAAGTATGTTGCAGC & \centering (GT)17 & \centering 145-191 & \centering 0.2 & \centering	55 & \centerin	& \centering GATTGTGCAAATGGAGACA GTTTTTGAGATGACTCCTGTGGG & \centering (CA)24 & \centering 201-223 & \centering 0.125/0.075/0.05	& \centering 55	& \centering 800  \tabularnewline
 TexVet7 & \centering \citep{rooney1999microsatellite} - \citep{vollmer2012developing} & \centering GTCCTCTTTCCAGGTGTAGCC GCCCACTAAGTATGTTGCAGC & \centering (GT)17 & \centering 145-191 & \centering 0.2 & \centering	55 & \centerin	& \centering TGCACTGTAGGGTGTTCAGCAG CTTAATTGGGGGCGATTTCAC & \centering (CA)12 & \centering 162-178 & \centering 0.2	& \centering 55	& \centering 800  \tabularnewline
 Ttr04 & \centering \citep{rosel2005isolation} & \centering CTGACCAGGCACTTTCCAC GTTTGTTTCCCAGGATTTTAGTGC	& \centering (CA)25	& \centering 106-128 & \centering 0.125/0.075/0.05	& \centering 60	& \centering 700 \tabularnewline
 Ttr11 & \centering \citep{rosel2005isolation} & \centering CTTTCAACCTGGCCTTTCTG GTTTGGCCACTACAAGGGAGTGAA & \centering	(CA)21 & \centering	194-226 & \centering 0.125/0.075/0.05 & \centering	62	& \centering 700 \tabularnewline
 Ttr19 & \centering \citep{rosel2005isolation} & \centering TGGGTGGACCTCATCAAATC GTTTAAGGGCTGTAAGAGG	& \centering (CA)17	& \centering 174-202 & \centering 0.125/0.075/0.05 & \centering	60	& \centering 700 \tabularnewline
 Ttr34 & \centering \citep{rosel2005isolation} & \centering GCACATGAGTATGTGGACAGG GTTTCCTCCTTGGGAGTGTCCTCT & \centering (CA)19 & \centering 182-204 & \centering 0.125/0.075/0.05 & \centering	58	& \centering 700 \tabularnewline
 Ttr48	& \centering \citep{rosel2005isolation} & \centering AAGAGGATGCAAATGGCAAG GTTTGGTAAGAAAATACCAAAGTCC & \centering (CA)18 & \centering 132-144 & \centering 0.125/0.075/0.05	& \centering 58	& \centering 700 \tabularnewline
 Ttr58	& \centering \citep{rosel2005isolation} & \centering TGGGTCTTGAGGGGTCTG GTTTGCTGAGGCTCCTTGTTGG	& \centering (CA)17	& \centering 168-196 & \centering 0.125/0.075/0.05	& \centering 60	& \centering 700 \tabularnewline
...
%\bibliography {BibliographieEA, Bibliographie}
