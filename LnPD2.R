...
# Plot
gg4 <- qplot(dfr$K, dfr$mean) + scale_x_discrete(limit = c(dfr$K)) + geom_errorbar(aes(x=dfr$K, ymin=dfr$mean-dfr$sd, ymax=dfr$mean+dfr$sd), width=0.1) + geom_line(colour = "blue") + labs(x = "K", y = "LnPD") + theme(legend.position="none")
